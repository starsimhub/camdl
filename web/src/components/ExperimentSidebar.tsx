...
export default function ExperimentSidebar() {
  ...
  // IR param defaults for the param editor
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-surface-1 border-r border-surface-border">
      ...
      {selected && (
        <div className="flex-1 overflow-y-auto px-3 py-2 border-b border-surface-border min-h-0">
          {/* Scenario name (editable for non-baseline) */}
          ...
          {/* Param overrides */}
          {irParams.length === 0 && (
            <div className="text-xs text-gray-600 italic">Compile a model to edit parameters.</div>
          )}
            ...
            <>
              ...
              <div className="flex flex-col gap-1">
                {irParams.map((p) => {
                  ...
                  // p.value may be null when the DSL has no default for this param
                  const irValue = (p.value as number | null);
                  // For non-baseline: effective value is override → baseline override → IR value
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="flex-1 text-xs bg-surface-2 border border-surface-border text-gray-300 rounded px-1.5 py-0.5 focus:outline-none"
            >
          ...
          {(runConfig.backend === 'tau_leap' || runConfig.backend === 'chain_binomial') && (
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-20 text-xs bg-surface-2 border border-surface-border text-gray-300 rounded px-1.5 py-0.5 focus:outline-none"
                step="0.1" min="0.01"
              />
            </div>
          )}
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-16 text-xs bg-surface-2 border border-surface-border text-gray-300 rounded px-1.5 py-0.5 focus:outline-none"
              min="1"
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              placeholder="IR default"
              onChange={(e) => {
                const v = parseFloat(e.target.value);
                setRunConfig({ tEnd: isNaN(v) ? undefined : v });
              }}
              className="w-20 text-xs bg-surface-2 border border-surface-border text-gray-300 rounded px-1.5 py-0.5 focus:outline-none placeholder-gray-600"
              min="0"
            />
          </div>
        </div>
            ...
            {scenarios.map((sc) => (
              sc.seedsCompleted > 0 ? (
                <div key={sc.id} className="flex items-center gap-1.5">
                  ...
                  <div className="flex-1 h-0.5 bg-surface-2 rounded">
                    <div
                      className="h-full rounded transition-all"
                      style={{
                  ...
                  <span className="text-xs text-gray-600">{sc.seedsCompleted}</span>
                </div>
              ) : null
            ))}
          </div>
        ...
        {scenarios.some((s) => s.status === 'error') && (
          <div className="mt-2">
            {scenarios.filter((s) => s.status === 'error').map((sc) => (
              <div key={sc.id} className="text-xs text-red-400 mt-1">
                {sc.name}: {sc.error}
              </div>
            ))}
          </div>
