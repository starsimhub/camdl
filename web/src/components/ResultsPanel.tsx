...
import { buildViews, type EnsembleMode, findDynamicEndIndex, TRACE_THRESHOLD } from "../lib/buildViews";
  ...
  if (visible.length <= LEGEND_GROUP_THRESHOLD) {
    return (
      ...
      >
        {visible.map((e) => (
          <span
            key={typeof e.dataKey === "string" ? e.dataKey : e.value}
            style={{ display: "flex", alignItems: "center", gap: 5, color: "var(--text-md)" }}
          >
            <span
              style={{
                ...
                width: 16,
                height: 2,
                background: e.color ?? "var(--text-lo)",
                borderRadius: 1,
              }}
            />
            ...
            {isOpen && (
              <div style={{ paddingLeft: 21, display: "flex", flexDirection: "column", gap: 1 }}>
                {entries.map((e) => (
                  <span
                    key={typeof e.dataKey === "string" ? e.dataKey : e.value}
                    style={{ color: "var(--text-md)", display: "flex", alignItems: "center", gap: 4 }}
                  >
                    <span
                      style={{
                        display: "inline-block",
        ...
        <div className="flex-1 min-h-0 px-2 py-3">
          {!hasResults && !isRunning && (
            <div className="flex flex-col items-center justify-center h-full gap-2">
              ...
              <span className="text-gray-400 text-xs dark:text-gray-700">
                Load an example from the header if parameters have no defaults
              </span>
            </div>
          )}
          {!hasResults && isRunning && (
